 ...
 *    - GDPR deletion (needs to access all player data)
 *    NEVER expose this client to browser code.
 *
 * 2. Player Client — respects RLS. Used for:
 *    - API routes handling player requests
 *    - Passes the player's Cognito JWT so RLS policies can check auth.uid()
...
/**
 * Get a Supabase client that operates in the context of a specific player.
 * Passes the player's JWT as the Authorization header so RLS policies
 * can enforce data isolation via auth.uid().
 *
 * This is the Cognito → Supabase JWT bridge.
 *
 * @param accessToken — The player's Cognito JWT (already verified by middleware)
 */
export function getSupabasePlayerClient(accessToken: string): SupabaseClient<Database> {
  const supabaseUrl = process.env.NEXT_PUBLIC_SUPABASE_URL;
