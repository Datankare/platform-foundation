...
Ten capabilities were missing from the roadmap. Each is assigned to the earliest phase where it can be meaningfully delivered, with infrastructure in PF and application-specific use in consumer repos.

#### 1. AI Output Screening (Phase 2)

**Gap:** ADR-016 screens every input. But AI-generated content — onboarding dialogue, admin command bar responses, dynamic content — goes directly to the user with no safety screening.
...
**Gap:** The orchestrator is request/response only. Every conversational surface waits for the full response. For a platform where AI is the interaction medium, that latency kills the experience.
...
**Gap:** The AI operates in English. Safety classification, admin commands, and onboarding are all English-first. A Spanish-speaking user gets English-classified safety decisions and English AI help translated after the fact.

**Requirement:** Safety classification operates in the user's language. AI interactions (onboarding, support, contextual help) are generated in the user's language natively, not translated post-hoc.
...
#### 5. User AI Context Store (Phase 4)

**Gap:** RAG in Phase 4 injects documents into prompts. But there's no persistent AI memory per user. Each AI interaction starts cold. The AI won't remember that this user struggled with previous tasks or prefers certain interaction styles.

**Requirement:** A per-user context store that accumulates AI-relevant signals: learning patterns, preference signals, interaction history, skill assessments. This context is injected into AI prompts alongside RAG content.

**Placement:** Phase 4 (alongside RAG + embedding store). User context stored as embeddings in pgvector. Context injection is a standard parameter in the orchestrator: `userContext?: string`.

#### 6. AI Output Explainability (Phase 4)

**Gap:** The audit trail logs what happened (input hash, classification, action) but not why in a way useful to admins, users filing appeals, or developers debugging AI behavior.

**Requirement:** AI decisions include an explanation chain: why was this content blocked? Why did the AI adjust behavior? Why did personalization change the experience? Explanations stored alongside audit records.

**Placement:** Phase 4 (Content Safety Foundation — human review queue needs explainability). The ClassifierOutput gains an `explanation` field. Admin moderation UI displays the explanation chain. Users see a human-readable reason on content blocks.

#### 7. Agentic Workflow Framework (Phase 5)

**Gap:** The admin command bar is plan→confirm→execute — one agentic pattern. No general-purpose agent framework exists. Complex AI workflows need it — multi-step reasoning, dispute resolution (gather evidence → analyze → recommend), and anti-cheat (detect → correlate → escalate).
...
**Placement:** Phase 5 (Application Framework — complex app workflows are the first consumer). `platform/ai/agent.ts` — agent runner with tool registry, step state, rollback.

#### 8. Multimodal AI (Phase 5)

**Gap:** The AI stack is text-in/text-out. Applications should understand images, generate visual content, and process audio natively (not just transcribe-then-classify).
...
**Placement:** Phase 5 (Application Framework — apps need visual and audio AI). The `AIProvider` interface gains multimodal content blocks. `AIMessage.content` supports `image` and `audio` block types alongside text.
...
#### 10. User Feedback Loop (Phase 7)

**Gap:** Phase 7 mentions "user satisfaction signals" but doesn't specify how user feedback flows back into AI improvement. Without a closed loop, AI quality is static after deployment.

**Requirement:** Users can rate AI responses (thumbs up/down). Appeal outcomes feed back into classifier training data. Translation corrections improve prompt instructions. All feedback is tracked, aggregated, and surfaced in quality dashboards.
...
| 4     | RAG pipeline, embedding store, user AI context, output explainability                          |
| 5     | Adaptive AI behavior, content generation, app-specific RAG, agentic framework, multimodal      |
| 6     | Token budgets, cost attribution per tier, AI A/B testing                                       |
| 7     | AI quality monitoring, NL analytics, personalization, user feedback loop, cost dashboards      |
| 8     | Conversational onboarding framework, contextual AI support, anomaly detection                  |
...
- Phase 4 scope increases: add user context store, add output explainability
- Phase 5 scope increases: add agentic framework, add multimodal provider interface
- Phase 6 scope increases: add A/B testing
- Phase 7 scope increases: add user feedback loop
