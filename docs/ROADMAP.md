...
| 8.2.0 | 2026-09-10 | Raman Sud | Phase 5 Sprint 3d complete. Agent registry unification (ADR-039); escalation SLA + remedial reaper (ADR-041); held-action & dual-control admin (ADR-040) — safety_approver peer role (migration 034), unified approvals queue, decision route with server-side independent-and-authorized enforcement (self-approval blocked even for super_admin), dual-control-keys management, approve-by-conversation behind a mandatory confirm. Admin surface brought fully into the coverage map (no blanket ignores); floors ratcheted to statements 88 / lines 90 / functions 90 / branches 76. 218 suites, 2,730 tests. PF v2.1.0 (minor). |
| 8.3.0 | 2026-09-19 | Raman Sud | Phase 5 Sprint 4 complete. Adaptive behavior (ADR-036) + dynamic content generation (ADR-037) frameworks on the ADR-038 prompt-eval harness. Content runs on the governed runtime, schema-validated, Guardian-screened before it surfaces, fail-closed to a static template; Curator is the reference content type; L21 conformance kits for both. 231 suites, 2,858 tests, 90.52% coverage. PF v2.2.0 (minor). |
| 8.4.0 | 2026-09-22 | Raman Sud | Phase 5 Sprint 5 complete. Application-specific RAG (ADR-042) + UGC input screening (ADR-043). Store-agnostic isolation with named-dimension scope + KB registry; in-memory and durable Supabase/pgvector stores (app-layer enforcement, optional RLS backstop); store-agnostic no-leak L21 kit; curator reference KB on the boot path. Input screening structural + fail-closed at ingest and query on the provider-agnostic seam, L21 kit. 237 suites, 2,895 tests, 90.52% coverage. PF v2.3.0 (minor). |
| 8.5.0 | 2026-09-24 | Raman Sud | Phase 5 Sprint 6 complete. Multimodal AI (ADR-044/045/046/047). Multimodal provider interface (image/audio content blocks + per-modality capability descriptor + fail-closed degrade-to-text); screenModality seam (both directions, independent NSFW/real-person/baseline axes, CSAM + real-person sexual imagery hard-refused above the tiers); screened multimodal input; governed image generation (draft -> screen -> risk-classify -> auto-commit / held-for-Confirm, idempotent, provenance-credentialed); provenance emit (C2PA-shaped, verifiable) + synthetic-origin detection as a tier-raising signal; held generations enqueue in the admin queue. One write-once substrate; L21 kits throughout. 246 suites, 2,949 tests, 90.58% coverage. PF v2.4.0 (minor). |
| 8.6.0 | 2026-09-24 | Raman Sud | Phase 5 Sprint 6.5 complete (maintenance & governance hardening). ADR-048 governed agent budget & durability config Accepted (per-agent cost/step caps as a min-wins platform ceiling, fail-safe to default; durable trajectory/budget stores required in production, fail-closed). Overdue debt cleared: TASK-060 promotion-guard CI on both repos (develop→main un-mergeable wrong-base); TASK-080 coverage ratchet with hysteresis (committed baseline, auto-raise, never auto-lower); TASK-064 boundary-map conformance arm; TASK-070 dependency-override register + drift guard; TASK-058 scheduled npm audit sweep (code/CI half); TASK-050 jest global soft-delete disabled on Playform. 247 suites, 2,961 tests, 90.66% coverage (floors held). PF v2.5.0 (minor); Playform v0.5.0. Exit deps TASK-046 (live k6) and TASK-025 (ffmpeg ALB) remain open. |

_Last updated: 2026-09-19 (Phase 5 Sprint 4 complete; changelog 8.3.0)_
