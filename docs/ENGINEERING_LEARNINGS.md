...
**86. A green local gate is not a green release — the CI production dependency audit is a hard gate the local gate never runs, and version stamps drift beyond README.** Cutting v2.1.0, every local commit-script gate passed, but CI's `Layer 0d — Dependency audit (production)` failed on two freshly-published Next.js advisories (unauthenticated RCE) against the pinned `next@16.2.12` — nothing of ours introduced it, and `format`/`tsc`/`lint`/`jest`/`coverage` never see it. Separately, the pre-tag doc sweep found `docs/TAD.md`'s `**Version:**` stamp still at 2.0.0 while `package.json`, README, and RELEASE_NOTES read 2.1.0. Rules: (1) before promoting a release, run `npm audit --omit=dev --audit-level=high` locally — an upstream advisory can block the release with none of your code at fault, and the local gate does not run it (extends Gotcha 81 — CI-only failures the type and unit gates cannot see); (2) at release, sweep every version-bearing surface, not just README + RELEASE_NOTES — `docs/TAD.md`'s Version stamp, the roadmap phase-status cells, and SETUP's example pin all carry the version and drift silently (extends Gotcha 82's verify-before-tag); (3) a version stamp tracks the release and IS bumped, while a `_Last reviewed:` footer tracks the last content change and is NOT bumped for a release that did not change the doc (Gotcha 83) — keep the two distinct.

**87. Advancing Claude’s sandbox by hand between steps drifts it from `develop` — take a fresh archive per build step, and reconstruct intermediate state by running the commit-scripts, never by editing.** Between commit-scripts in one session, Claude’s sandbox tree was moved forward by hand to mirror what a just-delivered step would produce, twice with consequences: (1) a hand-synced multi-line `PENDING_EVAL` array was matched against the one-line array prettier had actually produced on `develop`, so the next step’s anchor found nothing — the `.count(...) == 1` guard refused cleanly, no bad apply, but a round trip was lost; (2) a `VALIDATE_ONLY=1` sync appeared to succeed yet left no files applied, so a later step built on a silently stale base. The common cause is that a hand-mutated sandbox is a guess at the repo’s real bytes, and the two diverge in exactly the whitespace and formatting the anchors depend on. Rules: (1) each build step that reads or edits the tree starts from a fresh `git archive HEAD` of `develop` (§0), never a sandbox carried over from the previous step; (2) when intermediate state is genuinely needed, reconstruct it by running the delivered commit-scripts in order — they produce prettier-correct bytes — not by hand edits; (3) a sandbox sync that reports success is not proof files landed — assert the applied content, since a clean `VALIDATE_ONLY` exit says the gate passed, not that the tree changed (extends Gotcha 72 — a claim about state is a query).

**88. Prettier collapses a short array or object literal onto one line — anchor on the formatted output on `develop`, not the shape you authored.** The `PENDING_EVAL` anchor miss in Gotcha 87 had a single mechanical cause: a two- or three-element array written multi-line in source is reformatted by prettier to one line (`const PENDING_EVAL = []`, `[a, b, c]`) once it fits the print width, so an anchor built from the multi-line shape matches nothing in the committed file. The same applies to short object literals and import lists. Rules: (1) before building an anchor over any array/object/import that prettier controls, take its bytes from the formatted file on `develop`, not from how it was typed; (2) when in doubt, run the formatter in the sandbox and read the exact line back, because the print-width threshold is not obvious by eye; (3) this is Gotcha 71’s “the formatter’s version is the check” applied to whitespace — the same pinned prettier that gates the repo decides the one-line-vs-multi-line shape your anchor must match.

**89. `AgentConfig` requires `tools` — build a config from the interface declaration, not from the subset a previous agent set, even for a throwaway test host.** Building a host agent to exercise the adaptive loop, an `AgentConfig` written with `id`/`name`/`description`/`budgetConfig` failed `tsc` because `tools` is a required field, not optional — even when the test agent registers none (`tools: []` is the minimum). It is easy to omit because a host that only runs a workflow feels tool-less. Rules: (1) the required `AgentConfig` set is `id`, `name`, `description`, `budgetConfig`, and `tools` — construct from the interface, not from a call site that happened to set a subset; (2) an empty capability is still a declared field: prefer `tools: []` over omission so the shape is explicit; (3) this is Gotcha 78/76’s “prefer the declaration over the proxy” applied to config construction — read `AgentConfig`, don’t infer it from a neighbour.**

**90. A framework barrel that re-exports a reference implementation drags the reference's heavy imports into every consumer of the barrel — export the reference from its own path.** Activating the adaptive reference, its bootstrap (`registerAdaptiveReference`, a value-import of the `@/platform/agents` barrel) was briefly re-exported from `platform/adaptive/index.ts`. That made every importer of the adaptive barrel — including framework tests that only wanted `routeAdaptiveEffect` — eagerly load the agents graph and, through it, `@/platform/action-pipeline`. A pre-existing, unchanged sibling test (`adaptive-effect`) then failed: its `jest.mock('@/platform/action-pipeline', ...)` partial mock no longer intercepted, the real pipeline ran, and it hit an `{} as never` stub (`req.stateStore.commit is not a function`). The barrel is a wide fan-in; whatever it eagerly imports, every consumer pays for, including their module-graph and mocking assumptions. The fix kept `index.ts` framework-only and imported the bootstrap/behavior from their specific paths (`@/platform/adaptive/bootstrap`, `@/platform/adaptive/behaviors/pacing`). Rules: (1) a framework barrel re-exports only the framework's own core; a reference or bootstrap that value-imports a heavy sibling barrel is imported from its own path, never surfaced through the barrel; (2) when a barrel change breaks an unrelated, unchanged test, suspect the module graph the barrel now pulls in before you suspect the test; (3) type-only imports are erased and safe to surface — it is value-imports of heavy barrels that leak. Related to Gotcha 43 (module-singleton isolation): both are failures of reasoning about the module graph rather than the code in front of you.

**91. A commit-script that stages with `git add -A` commits whatever else is lying in the working tree — stage only the paths the script touched.** ADR-037 step 4's commit-script gated clean and committed, but its commit stage ran `git add -A`, which swept two untracked archives sitting in the repo root (`pf.tgz`, `repo.tgz`) into the commit and pushed ~2.2 MiB of debris to `develop`. The clean-tree preflight ran `git status --porcelain --untracked-files=no` and correctly ignored them — tolerating untracked debris is deliberate, the script's own tarball and the human's stray files are not its concern — but `git add -A` at commit time adopted exactly what the preflight had waved through. The two decisions must agree: a preflight that permits untracked files forbids a blanket add. Rules: (1) the commit stage adds only the explicit set of paths the script created or edited (`git add <the declared NEW + EDITED paths>`), never `git add -A` or `git add .` — a targeted add cannot sweep a stray file no matter what else is present; (2) this is independent of `.gitignore` — gitignoring the known debris (here `*.tgz`) is worth doing but is belt to the targeted-add's suspenders, not a substitute; (3) the tell is a `git commit` summary whose file count exceeds what the script declares — read that summary before pushing, or assert committed-paths == declared-set.

**92. Registering a graph-heavy module in `jest.setup` pre-binds its imports before any test's `jest.mock` runs — register in the test's own `beforeAll`, and mock at the seam with a partial mock.** Wiring the content framework's `registerSocialContentTypes()` into `jest.setup.ts` (the file mirrors prod boot registrations) imported `curator-content` at setup time, which eagerly pulled the whole `content → loop → @/platform/ai` graph. `setupFilesAfterEnv` runs BEFORE a test file's hoisted `jest.mock` factories are registered, so that graph was cached bound to the REAL orchestrator; the test's later `jest.mock('@/platform/ai')` then resolved a different instance, `generateContent` called the real provider (`ANTHROPIC_API_KEY not configured`), and the mock's call count stayed zero — an hour lost to a two-instance mock. Rules: (1) do not register in `jest.setup` a module whose import drags the generation/AI graph — register content types (and anything similar) in the specific test's `beforeAll`, so only the tests that need it load it and their mocks are not pre-empted; unlike agents, which many tests need globally present, content types are needed only by their own tests, so global pre-registration buys nothing and costs mockability; (2) mock at the seam with a PARTIAL mock — `{ ...jest.requireActual(m), theFn: jest.fn() }` — so consumers keep the module's other real exports and the override still reaches them (the adaptive and content conformance kits do exactly this and their orchestrator/screening mocks land); (3) when a mock silently does not intercept (call count zero, the real dependency runs), suspect a second module instance from setup-time or barrel-time pre-binding before the mock — this is Gotcha 90's graph-leak and Gotcha 73's silent fallback in a new guise.
...
_Last updated: September 10, 2026 (Phase 5 Sprint 3d CLOSE — Gotcha 86 added; the CI production dependency audit is a release gate the local gate never runs, and version stamps drift beyond README)_
_Last updated: September 16, 2026 (Phase 5 Sprint 4 — Gotchas 87–89 added; sandbox drift needs a fresh develop archive per step, prettier collapses short array/object literals to one line, AgentConfig requires tools)_
_Last updated: September 17, 2026 (Phase 5 Sprint 4 — Gotcha 90 added; a framework barrel re-exporting a reference that value-imports a heavy sibling barrel drags that graph into every consumer and breaks their mocks — export the reference from its own path)_
_Last updated: September 19, 2026 (Phase 5 Sprint 4 — Gotchas 91-92 added; a commit-script stages only the paths it touched, never `git add -A`, and registering a graph-heavy module in jest.setup pre-binds it before per-test mocks — register in the test's own beforeAll with partial mocks)_
